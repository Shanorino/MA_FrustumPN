...
def demo():
    ...
    #pc_velo = dataset.get_image(data_idx)
    ...
    print(' -------- all LiDAR points. Draw 3d box in LiDAR point cloud --------')
    ...
    box3d_pts_2d, box3d_pts_3d = utils.compute_box_3d(objects[1], calib.P) 
    ...
    print ("Dumping data: ", box3droi_pc_velo)
    ...
    print("Dumping data: ", boxfov_pc_velo)
...
def extract_frustum_data(idx_filename, split, output_filename, viz=False,
    ...
    frustum_angle_list = [] # angle of 2d box center from pos x-axis. Used for ROTATION (data)!!!

    pos_cnt = 0
    all_cnt = 0
    for data_idx in data_idx_list:
        print('------------- ', data_idx)
        calib = dataset.get_calibration(data_idx) # calibration class
        objects = dataset.get_label_objects(data_idx)
        ...
        for obj_idx in range(len(objects)): # iterate for each (labeled) object
